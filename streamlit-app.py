...
def create_documents(content, file_name):
    ...
    documents = []
    if content.strip():
        documents.append(Document(content=content, doc_id=file_name))
    return documents
                ...
                if file_key not in st.session_state.get('file_cache', {}):
                    if os.path.exists(temp_dir):
                        if uploaded_file.type == "application/pdf":
                            ...
                            if not docs:
                                st.error('The uploaded file has no valid content to index. Please upload a valid CSV or XLSX file.')
                                st.stop()
                            display_file(file_path)
                    else:
